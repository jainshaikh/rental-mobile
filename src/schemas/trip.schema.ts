...
export const tripFormSchema = z.object({
  ...
  originCity: z
    .string()
    .min(2, 'Origin city is too short')
    .max(100, 'Origin city is too long'),
  destinationCity: z
    .string()
    .min(2, 'Destination city is too short')
    .max(100, 'Destination city is too long'),
  pickupPoint: z
    .string()
    .min(3, 'Pickup point is too short')
    .max(300, 'Pickup point is too long'),
  pickupLat: z.number().optional(),
  pickupLng: z.number().optional(),
  dropoffPoint: z
    .string()
    .max(300, 'Drop-off point is too long')
    .optional()
    .or(z.literal('')),
  dropoffLat: z.number().optional(),
  dropoffLng: z.number().optional(),
  departureAt: z
    .date({ message: 'Pick a departure date and time' })
    .refine((d) => d > new Date(), {
